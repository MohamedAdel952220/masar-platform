const { useState: useStateTAuth, useEffect: useEffectTAuth, useRef: useRefTAuth } = React;

/* ====================================================================
   Masar — Teacher App · Auth gate
   Splash → Onboarding (3 teacher scenes) → Login → Forgot password
   Exposes window.TeacherAuthGate({ lang, setLang, onDone })
   ==================================================================== */

const VIO = 'var(--role-teacher)';
const VIO_SOFT = '#EDE8F8';

/* ---------------- Language pill ---------------- */
function TLangPill({ lang, setLang, onDark }) {
  const ar = lang === 'ar';
  return (
...
/* ====================================================================
   SPLASH
   ==================================================================== */
function TSplash({ lang, onDone }) {
  const ar = lang === 'ar';
  useEffectTAuth(() => { const t = setTimeout(onDone, 2100); return () => clearTimeout(t); }, []);
  return (
    <div style={{ position: 'absolute', inset: 0, background: 'linear-gradient(160deg, #4B3B8F 0%, #3C2E78 60%, #2E2260 100%)', display: 'flex', flexDirection: 'column', alignItems: 'center', justifyContent: 'center', overflow: 'hidden' }}>
      <div style={{ position: 'absolute', top: '-12%', insetInlineStart: '-18%', width: 280, height: 280, borderRadius: '50%', background: 'rgba(239,159,39,.14)', filter: 'blur(8px)' }} />
      ...
      <div style={{ animation: 'tsplashPop .7s var(--ease-out) both', display: 'flex', flexDirection: 'column', alignItems: 'center', zIndex: 2 }}>
        ...
        <div style={{ marginTop: 6, fontSize: 14, color: 'rgba(245,243,238,.78)', fontWeight: 600, fontFamily: ar ? 'var(--font-arabic)' : 'var(--font-sans)' }}>{ar ? 'بوابة المعلمين' : 'Teacher Portal'}</div>
      </div>
      <div style={{ position: 'absolute', bottom: 70, display: 'flex', gap: 8, zIndex: 2 }}>
        {[0, 1, 2].map(i => <span key={i} style={{ width: 8, height: 8, borderRadius: '50%', background: '#F5F3EE', opacity: 0.5, animation: `tsplashDot 1.2s ${i * 0.18}s infinite` }} />)}
      </div>
    </div>
  );
}
...
/* ====================================================================
   ONBOARDING — 3 teacher scenes
   ==================================================================== */
/* Scene 1 — Roster & evaluate (student rows with rating dots) */
function TSceneRoster({ play, onReplay }) {
  const rows = [
    { n: 'Yousef', done: true }, { n: 'Lina', done: true }, { n: 'Malak', done: false },
  ];
  return (
    <div onClick={onReplay} style={{ cursor: 'pointer', width: 250, display: 'flex', flexDirection: 'column', gap: 10 }}>
      {rows.map((r, i) => (
        <div key={'r' + i + play} style={{ display: 'flex', alignItems: 'center', gap: 11, padding: '11px 13px', borderRadius: 'var(--radius-md)', background: 'var(--surface-card)', border: '1px solid var(--border-subtle)', boxShadow: 'var(--shadow-sm)', animation: `tbubbleIn .45s ${i * 0.18}s var(--ease-out)` }}>
          <Avatar name={r.n} size={34} />
          <span style={{ flex: 1, fontSize: 13.5, fontWeight: 700, color: 'var(--text-strong)' }}>{r.n}</span>
          {r.done
            ? <span style={{ display: 'flex', gap: 3 }}>{[0,1,2,3,4].map(d => <span key={d} style={{ width: 7, height: 7, borderRadius: '50%', background: d < 4 ? 'var(--amber-500)' : 'var(--neutral-300)' }} />)}</span>
            : <span style={{ fontSize: 11, fontWeight: 700, color: VIO, background: VIO_SOFT, padding: '3px 9px', borderRadius: 'var(--radius-pill)' }}>To eval</span>}
        </div>
      ))}
    </div>
  );
...
/* Scene 2 — AI report drafting (typing lines appearing in a card) */
function TSceneReport({ play, onReplay }) {
  return (
    <div onClick={onReplay} style={{ cursor: 'pointer', width: 250 }}>
      <div style={{ borderRadius: 'var(--radius-lg)', background: 'var(--surface-card)', border: '1px solid var(--border-subtle)', boxShadow: 'var(--shadow-md)', padding: 16 }}>
        <div style={{ display: 'flex', alignItems: 'center', gap: 9, marginBottom: 12 }}>
          <span style={{ width: 34, height: 34, borderRadius: '50%', background: VIO_SOFT, color: VIO, display: 'grid', placeItems: 'center' }}><Icon name="sparkles" size={18} /></span>
          <span style={{ fontSize: 13, fontWeight: 800, color: 'var(--text-strong)' }}>AI draft</span>
          <span style={{ marginInlineStart: 'auto', fontSize: 11, fontWeight: 700, color: VIO, background: VIO_SOFT, padding: '3px 9px', borderRadius: 'var(--radius-pill)' }}>June</span>
        </div>
        {[0.85, 1, 0.7].map((w, i) => (
          <div key={'l' + i + play} style={{ height: 9, borderRadius: 5, background: 'var(--neutral-200)', marginBottom: 9, width: `${w * 100}%`, transformOrigin: 'left', animation: `growLine .5s ${0.2 + i * 0.3}s var(--ease-out) both` }} />
        ))}
        <div key={'b' + play} style={{ marginTop: 6, display: 'inline-flex', alignItems: 'center', gap: 6, padding: '7px 12px', borderRadius: 'var(--radius-pill)', background: VIO, color: '#fff', fontSize: 12, fontWeight: 700, animation: `tbubbleIn .4s 1.2s var(--ease-out) both` }}>
          <Icon name="send" size={13} color="#fff" />Send to parent
        </div>
      </div>
    </div>
  );
...
/* Scene 3 — Chat & flag concerns */
function TSceneConnect({ play, onReplay }) {
  return (
    <div onClick={onReplay} style={{ cursor: 'pointer', width: 250, display: 'flex', flexDirection: 'column', gap: 10 }}>
      <div key={'a' + play} style={{ alignSelf: 'flex-end', maxWidth: '76%', background: VIO, color: '#fff', borderRadius: '16px 16px 4px 16px', padding: '10px 14px', boxShadow: 'var(--shadow-sm)', animation: 'tbubbleIn .5s .05s var(--ease-out)' }}>
        <div style={{ fontSize: 13 }}>Yousef did great in English today! 🌟</div>
      </div>
      <div key={'b' + play} style={{ alignSelf: 'flex-start', maxWidth: '70%', background: 'var(--surface-card)', border: '1px solid var(--border-subtle)', borderRadius: '16px 16px 16px 4px', padding: '10px 14px', boxShadow: 'var(--shadow-sm)', animation: 'tbubbleIn .5s .35s var(--ease-out)' }}>
        <div style={{ fontSize: 12.5, fontWeight: 700, color: 'var(--teal-700)', marginBottom: 2 }}>Parent</div>
        <div style={{ fontSize: 13, color: 'var(--text-body)' }}>Thank you so much! 💚</div>
      </div>
      <div key={'c' + play} style={{ alignSelf: 'center', marginTop: 4, display: 'flex', alignItems: 'center', gap: 9, padding: '9px 14px', borderRadius: 'var(--radius-md)', background: 'var(--amber-50)', border: '1px solid var(--amber-200)', animation: 'tbubbleIn .5s .6s var(--ease-out)' }}>
        <Icon name="flag" size={16} color="var(--amber-700)" />
        <span style={{ fontSize: 12.5, fontWeight: 700, color: 'var(--amber-700)' }}>Flag a concern</span>
      </div>
    </div>
  );
}
...
const T_ONB = [
  { scene: 'roster', titleEn: 'Your classes, all in one place', titleAr: 'فصولك كلها في مكان واحد', subEn: 'See every class roster and evaluate each student’s understanding, participation and behavior.', subAr: 'اطّلع على قوائم فصولك وقيّم فهم كل طالب ومشاركته وسلوكه.' },
  { scene: 'report', titleEn: 'Write reports in seconds', titleAr: 'اكتب التقارير في ثوانٍ', subEn: 'Let AI draft daily notes and monthly reports from your evaluations — then send to parents.', subAr: 'دع الذكاء الاصطناعي يصيغ الملاحظات اليومية والتقارير الشهرية من تقييماتك — ثم أرسلها للأهل.' },
  { scene: 'connect', titleEn: 'Stay close to every family', titleAr: 'ابقَ قريبًا من كل عائلة', subEn: 'Chat with parents, flag concerns early, and request events, trips and exams.', subAr: 'تواصل مع الأهل، وارفع الملاحظات مبكرًا، واطلب الفعاليات والرحلات والامتحانات.' },
];

function TOnboarding({ lang, setLang, onDone }) {
  const ar = lang === 'ar';
  const [i, setI] = useStateTAuth(0);
  const [play, setPlay] = useStateTAuth(0);
  const step = T_ONB[i];
  const next = () => { if (i < T_ONB.length - 1) { setI(i + 1); setPlay(p => p + 1); } else onDone(); };
    ...
    <div style={{ position: 'absolute', inset: 0, background: 'var(--bg-app)', display: 'flex', flexDirection: 'column' }}>
      <div style={{ padding: '46px 18px 0', display: 'flex', alignItems: 'center', justifyContent: 'space-between' }}>
        ...
        <div style={{ display: 'flex', alignItems: 'center', gap: 10 }}>
          <TLangPill lang={lang} setLang={setLang} />
      ...
      <div style={{ flex: 1, display: 'flex', flexDirection: 'column', alignItems: 'center', justifyContent: 'center', padding: '0 28px' }}>
        <div style={{ height: 220, display: 'flex', alignItems: 'center', justifyContent: 'center', marginBottom: 8 }}>
          {step.scene === 'roster' && <TSceneRoster play={play} onReplay={() => setPlay(p => p + 1)} />}
          {step.scene === 'report' && <TSceneReport play={play} onReplay={() => setPlay(p => p + 1)} />}
          {step.scene === 'connect' && <TSceneConnect play={play} onReplay={() => setPlay(p => p + 1)} />}
        </div>
        ...
        <h2 key={'t' + i} style={{ margin: 0, fontSize: 23, fontWeight: 800, color: 'var(--text-strong)', textAlign: 'center', letterSpacing: '-.01em', fontFamily: ar ? 'var(--font-arabic)' : 'var(--font-sans)', animation: 'tbubbleIn .4s var(--ease-out)' }}>{ar ? step.titleAr : step.titleEn}</h2>
        <p key={'s' + i} style={{ margin: '12px 0 0', fontSize: 14.5, lineHeight: 1.6, color: 'var(--text-muted)', textAlign: 'center', maxWidth: 320, fontFamily: ar ? 'var(--font-arabic)' : 'var(--font-sans)', animation: 'tbubbleIn .4s .08s var(--ease-out)' }}>{ar ? step.subAr : step.subEn}</p>
      </div>
      <div style={{ padding: '0 24px 36px' }}>
        <div style={{ display: 'flex', justifyContent: 'center', gap: 8, marginBottom: 22 }}>
          {T_ONB.map((_, j) => (
            <button key={j} onClick={() => { setI(j); setPlay(p => p + 1); }} style={{ width: j === i ? 26 : 8, height: 8, borderRadius: 4, border: 'none', background: j === i ? VIO : 'var(--neutral-300)', cursor: 'pointer', transition: 'all .25s' }} />
          ))}
        </div>
        <div style={{ display: 'flex', gap: 12, alignItems: 'center' }}>
          ...
          <button onClick={next} style={{ flex: 1, height: 52, borderRadius: 'var(--radius-pill)', border: 'none', background: VIO, color: '#fff', fontFamily: ar ? 'var(--font-arabic)' : 'var(--font-sans)', fontSize: 16, fontWeight: 800, cursor: 'pointer', display: 'flex', alignItems: 'center', justifyContent: 'center', gap: 8 }}>
            {i === T_ONB.length - 1 ? (ar ? 'ابدأ الآن' : 'Get started') : (ar ? 'التالي' : 'Next')}
...
/* ====================================================================
   LOGIN
   ==================================================================== */
function TPhoneField({ value, onChange }) {
  return (
...
function TLogin({ lang, setLang, onLogin, onForgot }) {
  const ar = lang === 'ar';
  const [phone, setPhone] = useStateTAuth('');
  const [pw, setPw] = useStateTAuth('');
  const [show, setShow] = useStateTAuth(false);
  const [busy, setBusy] = useStateTAuth(false);
  const [error, setError] = useStateTAuth('');
  ...
  return (
    <div style={{ position: 'absolute', inset: 0, background: 'var(--bg-app)', display: 'flex', flexDirection: 'column', overflowY: 'auto' }}>
      ...
      <div style={{ padding: '24px 26px 30px', flex: 1, display: 'flex', flexDirection: 'column' }}>
        <div style={{ width: 60, height: 60, borderRadius: 18, background: VIO_SOFT, display: 'grid', placeItems: 'center', marginBottom: 18 }}>
          <Icon name="graduation-cap" size={30} color={VIO} />
        </div>
        <h1 style={{ margin: 0, fontSize: 26, fontWeight: 800, color: 'var(--text-strong)', letterSpacing: '-.02em', fontFamily: ar ? 'var(--font-arabic)' : 'var(--font-sans)' }}>{ar ? 'أهلًا بك مجددًا' : 'Welcome back'}</h1>
        <p style={{ margin: '8px 0 26px', fontSize: 14.5, color: 'var(--text-muted)', fontFamily: ar ? 'var(--font-arabic)' : 'var(--font-sans)' }}>{ar ? 'سجّل الدخول لإدارة فصولك' : 'Sign in to manage your classes'}</p>
        <label style={lbl}>{ar ? 'رقم الهاتف' : 'Phone number'}</label>
        <div style={{ marginBottom: 18 }}><TPhoneField value={phone} onChange={setPhone} /></div>
        ...
        <button onClick={onForgot} style={{ alignSelf: ar ? 'flex-start' : 'flex-end', background: 'none', border: 'none', color: VIO, fontSize: 13, fontWeight: 700, cursor: 'pointer', marginBottom: 26, fontFamily: ar ? 'var(--font-arabic)' : 'var(--font-sans)' }}>{ar ? 'نسيت كلمة المرور؟' : 'Forgot password?'}</button>
        ...
        <button onClick={submit} disabled={!valid || busy} style={{ height: 52, borderRadius: 'var(--radius-pill)', border: 'none', background: valid ? VIO : 'var(--neutral-300)', color: '#fff', fontFamily: ar ? 'var(--font-arabic)' : 'var(--font-sans)', fontSize: 16, fontWeight: 800, cursor: valid && !busy ? 'pointer' : 'not-allowed', display: 'flex', alignItems: 'center', justifyContent: 'center', gap: 8 }}>
        ...
        <div style={{ marginTop: 'auto', paddingTop: 24, textAlign: 'center', fontSize: 13.5, color: 'var(--text-muted)', fontFamily: ar ? 'var(--font-arabic)' : 'var(--font-sans)' }}>
          ...
          <span style={{ color: VIO, fontWeight: 700, cursor: 'pointer' }}>{ar ? 'تواصل مع الإدارة' : 'Contact admin'}</span>
        </div>
      </div>
    </div>
  );
...
/* ====================================================================
   FORGOT PASSWORD
   ==================================================================== */
function TOTPInput({ value, onChange, len = 4 }) {
  const refs = useRefTAuth([]);
  const set = (idx, v) => {
    ...
    <div style={{ display: 'flex', gap: 10, justifyContent: 'center', direction: 'ltr' }}>
      {Array.from({ length: len }).map((_, i) => (
        <input key={i} ref={el => refs.current[i] = el} type="tel" inputMode="numeric" maxLength={1} value={value[i] || ''} onChange={e => set(i, e.target.value)}
          ...
          style={{ width: 56, height: 64, textAlign: 'center', fontSize: 26, fontWeight: 800, fontFamily: 'var(--font-mono)', border: `1.5px solid ${value[i] ? VIO : 'var(--border-subtle)'}`, borderRadius: 'var(--radius-md)', color: 'var(--text-strong)', background: 'var(--surface-card)', outline: 'none' }} />
      ))}
    </div>
  );
}

function TForgot({ lang, setLang, onClose, onReset }) {
  const ar = lang === 'ar';
  const [step, setStep] = useStateTAuth('phone');
  const [phone, setPhone] = useStateTAuth('');
  const [channel, setChannel] = useStateTAuth('whatsapp');
  const [otp, setOtp] = useStateTAuth('');
  const [pw, setPw] = useStateTAuth('');
  const [pw2, setPw2] = useStateTAuth('');
  const [seconds, setSeconds] = useStateTAuth(45);
  const [busy, setBusy] = useStateTAuth(false);
  const [error, setError] = useStateTAuth('');
  useEffectTAuth(() => {
    if (step !== 'otp') return;
      ...
      {step === 'phone' && (
        <React.Fragment>
          ...
          <div style={{ padding: '18px 26px 30px', flex: 1, display: 'flex', flexDirection: 'column' }}>
            <span style={{ width: 56, height: 56, borderRadius: 16, background: VIO_SOFT, display: 'grid', placeItems: 'center', marginBottom: 18 }}><Icon name="key-round" size={26} color={VIO} /></span>
            ...
            <label style={lbl}>{ar ? 'رقم الهاتف' : 'Phone number'}</label>
            <div style={{ marginBottom: 24 }}><TPhoneField value={phone} onChange={setPhone} /></div>
            <button onClick={() => phone.length >= 10 && setStep('channel')} disabled={phone.length < 10} style={{ height: 52, borderRadius: 'var(--radius-pill)', border: 'none', background: phone.length >= 10 ? VIO : 'var(--neutral-300)', color: '#fff', fontFamily: ar ? 'var(--font-arabic)' : 'var(--font-sans)', fontSize: 16, fontWeight: 800, cursor: phone.length >= 10 ? 'pointer' : 'not-allowed' }}>{ar ? 'متابعة' : 'Continue'}</button>
          </div>
        </React.Fragment>
      )}
      {step === 'channel' && (
        <React.Fragment>
          ...
          <div style={{ padding: '18px 26px 30px', flex: 1, display: 'flex', flexDirection: 'column' }}>
            ...
            {[
              ...
              return (
                <button key={o.v} onClick={() => setChannel(o.v)} style={{ display: 'flex', alignItems: 'center', gap: 13, padding: '15px 16px', marginBottom: 12, borderRadius: 'var(--radius-md)', border: `1.5px solid ${on ? VIO : 'var(--border-subtle)'}`, background: on ? VIO_SOFT : 'var(--surface-card)', cursor: 'pointer', textAlign: 'start' }}>
                  ...
                  <span style={{ width: 22, height: 22, borderRadius: '50%', border: `2px solid ${on ? VIO : 'var(--border-strong)'}`, display: 'grid', placeItems: 'center', flex: 'none' }}>{on && <span style={{ width: 11, height: 11, borderRadius: '50%', background: VIO }} />}</span>
                </button>
              );
            })}
            {error && (
      ...
      {step === 'otp' && (
        <React.Fragment>
          ...
          <div style={{ padding: '18px 26px 30px', flex: 1, display: 'flex', flexDirection: 'column' }}>
            ...
            <TOTPInput value={otp} onChange={setOtp} len={4} />
            <div style={{ textAlign: 'center', marginTop: 22, fontSize: 13.5, color: 'var(--text-muted)', fontFamily: ar ? 'var(--font-arabic)' : 'var(--font-sans)' }}>
              {seconds > 0
                ...
                : <span onClick={() => setSeconds(45)} style={{ color: VIO, fontWeight: 700, cursor: 'pointer' }}>{ar ? 'إعادة إرسال الرمز' : 'Resend code'}</span>}
            </div>
            {error && (
            ...
            <button onClick={verifyCode} disabled={otp.length < 4 || busy} style={{ marginTop: 28, height: 52, borderRadius: 'var(--radius-pill)', border: 'none', background: (otp.length === 4 && !busy) ? VIO : 'var(--neutral-300)', color: '#fff', fontFamily: ar ? 'var(--font-arabic)' : 'var(--font-sans)', fontSize: 16, fontWeight: 800, cursor: (otp.length === 4 && !busy) ? 'pointer' : 'not-allowed' }}>{busy ? (ar ? 'جارٍ التحقق…' : 'Verifying…') : (ar ? 'تحقّق' : 'Verify')}</button>
          </div>
        </React.Fragment>
      )}
      {step === 'reset' && (
        <React.Fragment>
          ...
          <div style={{ padding: '18px 26px 30px', flex: 1, display: 'flex', flexDirection: 'column' }}>
            <span style={{ width: 56, height: 56, borderRadius: 16, background: VIO_SOFT, display: 'grid', placeItems: 'center', marginBottom: 18 }}><Icon name="lock-keyhole" size={26} color={VIO} /></span>
            ...
            <button onClick={savePassword} disabled={!pwValid || busy} style={{ marginTop: 28, height: 52, borderRadius: 'var(--radius-pill)', border: 'none', background: (pwValid && !busy) ? VIO : 'var(--neutral-300)', color: '#fff', fontFamily: ar ? 'var(--font-arabic)' : 'var(--font-sans)', fontSize: 16, fontWeight: 800, cursor: (pwValid && !busy) ? 'pointer' : 'not-allowed' }}>{busy ? (ar ? 'جارٍ الحفظ…' : 'Saving…') : (ar ? 'حفظ كلمة المرور' : 'Save password')}</button>
          </div>
        </React.Fragment>
      )}
      {step === 'done' && (
        <div style={{ flex: 1, display: 'flex', flexDirection: 'column', alignItems: 'center', justifyContent: 'center', padding: '0 30px', textAlign: 'center' }}>
          <span style={{ width: 88, height: 88, borderRadius: '50%', background: 'var(--success-50)', color: 'var(--success-600)', display: 'grid', placeItems: 'center', marginBottom: 22, animation: 'tsplashPop .5s var(--ease-out) both' }}><Icon name="circle-check-big" size={48} /></span>
          ...
          <button onClick={onReset} style={{ width: '100%', maxWidth: 280, height: 52, borderRadius: 'var(--radius-pill)', border: 'none', background: VIO, color: '#fff', fontFamily: ar ? 'var(--font-arabic)' : 'var(--font-sans)', fontSize: 16, fontWeight: 800, cursor: 'pointer' }}>{ar ? 'تسجيل الدخول' : 'Back to sign in'}</button>
        </div>
      )}
    </div>
  );
...
/* ====================================================================
   GATE
   ==================================================================== */
function TeacherAuthGate({ lang, setLang, onDone }) {
  const ar = lang === 'ar';
  const [stage, setStage] = useStateTAuth('splash');
  return (
    <div dir={ar ? 'rtl' : 'ltr'} style={{ position: 'absolute', inset: 0 }}>
      <style>{`
        @keyframes tsplashPop{from{opacity:0;transform:scale(.8)}to{opacity:1;transform:scale(1)}}
        @keyframes tsplashDot{0%,100%{opacity:.35;transform:translateY(0)}50%{opacity:1;transform:translateY(-4px)}}
        @keyframes tbubbleIn{from{opacity:0;transform:translateY(10px)}to{opacity:1;transform:translateY(0)}}
        @keyframes growLine{from{transform:scaleX(0)}to{transform:scaleX(1)}}
      `}</style>
      {stage === 'splash' && <TSplash lang={lang} onDone={() => setStage('onboarding')} />}
      {stage === 'onboarding' && <TOnboarding lang={lang} setLang={setLang} onDone={() => setStage('login')} />}
      {stage === 'login' && <TLogin lang={lang} setLang={setLang} onLogin={onDone} onForgot={() => setStage('forgot')} />}
      {stage === 'forgot' && <TForgot lang={lang} setLang={setLang} onClose={() => setStage('login')} onReset={() => setStage('login')} />}
    </div>
  );
}

window.TeacherAuthGate = TeacherAuthGate;
