...
- **API exposure**: PostgREST auto-API is enabled on client-facing schemas (`academic`, `approvals`, `transport`, `safety`, `billing`, `comms`, `media` — read/write per RLS), and on `platform`/`jobs` **specifically for the tables §12/§12.1/§13.6 give a human-role RLS policy to** (`platform.activity_log`, `platform.audit_log`, `platform.service_health_status`, `platform.support_tickets`, `platform.tenant_billing_transactions`, `jobs.scheduled_job_runs` — read per RLS, same "PostgREST + RLS is the read path" model as every other schema, §14). **Correction from an earlier draft of this section**: `platform`/`jobs` were previously described as excluded from the public API surface entirely, grouped with the machine-identity-relevant parts of `identity` (`service_accounts`) as a blanket "internal-only" belt-and-suspenders layer. That blanket exclusion was never reconciled against §13.6's own table-by-table `platform_admin` RLS policy design and §12.1's explicit requirement that tier enforcement be "an RLS policy branch... not an application-only check" — a requirement a service-role-only access path cannot satisfy without duplicating that logic in code. `service_accounts` and the two purely internal `jobs` tables (`background_job_queue`, `idempotency_keys`, neither of which has ever carried a human-role RLS policy) remain excluded from the public API surface entirely, reachable only via `service_role`-executed Edge Functions/RPCs, exactly as before.
