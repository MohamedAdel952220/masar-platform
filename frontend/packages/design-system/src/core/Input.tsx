 ...
 */
export function Input({ label, hint, error, iconLeft = null, id, style = {}, ...rest }: InputProps) {
  const [fallbackId] = useState(() => `in-${Math.random().toString(36).slice(2, 8)}`);
  const inputId = id || fallbackId;
  const [focus, setFocus] = useState(false);
  return (
    <div
      style={{ display: 'flex', flexDirection: 'column', gap: 6, fontFamily: 'var(--font-sans)', ...style }}
    >
      {label && (
        <label
          htmlFor={inputId}
      ...
      {(hint || error) && (
        <span
          style={{ fontSize: 'var(--text-xs)', color: error ? 'var(--danger-700)' : 'var(--text-muted)' }}
        >
          {error || hint}
        </span>
      )}
