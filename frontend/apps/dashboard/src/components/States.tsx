...
/**
 * Renders when the signed-in role lacks a write action. Distinct from
 * ErrorState: nothing failed — a teacher simply may not perform manager-only
 * actions (§12). RLS enforces this regardless of what the UI shows.
 */
export function ManagerOnly({ children }: { children?: ReactNode }) {
  return (
    <Card padding="md" accent="var(--amber-500)">
      <div style={{ display: 'grid', gap: 'var(--space-2)' }}>
        <Badge tone="amber">Manager access required</Badge>
        <span style={{ fontSize: 'var(--text-sm)', color: 'var(--text-muted)' }}>
          {children ?? 'Your account has read-only access to this area.'}
        </span>
      </div>
    </Card>
  );
