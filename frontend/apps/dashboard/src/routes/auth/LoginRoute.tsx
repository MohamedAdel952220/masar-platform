...
/**
 * Nursery Dashboard sign-in — phone + password (BACKEND_ARCHITECTURE.md §10.2).
 *
 * Tenant-side roles authenticate with a phone identity, unlike Platform Admin
 * (email + mandatory MFA). MFA is not required for tenant roles in v1, so there
 * is no second factor step here.
 *
 * No credential is created here. Accounts are provisioned by a manager and
 * activated through a one-time link sent to the account's own phone (§10.3).
 */
...
export function LoginRoute({
  ...
  const onSubmit = handleSubmit(async (values) => {
    ...
    try {
      ...
      // AuthGate re-renders on the session change and mounts the console.
    } catch (error) {
      setSubmitError(
        ...
        <form onSubmit={onSubmit} noValidate style={{ display: 'grid', gap: 'var(--space-5)' }}>
          <div style={{ display: 'grid', gap: 'var(--space-2)' }}>
            ...
            <p style={{ margin: 0, fontSize: 'var(--text-sm)', color: 'var(--text-muted)' }}>
              Sign in with the phone number registered for your nursery account.
            </p>
          </div>
