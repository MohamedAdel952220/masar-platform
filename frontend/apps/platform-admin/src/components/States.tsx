...
/**
 * Renders when a tier lacks an action. Distinct from ErrorState: nothing failed
 * — the account simply may not do this (§12.1).
 */
export function TierRestricted({ children }: { children?: ReactNode }) {
  return (
    <Card padding="md" accent="var(--amber-500)">
      <div style={{ display: 'grid', gap: 'var(--space-2)' }}>
        <Badge tone="amber">Owner or Admin tier required</Badge>
        <span style={{ fontSize: 'var(--text-sm)', color: 'var(--text-muted)' }}>
          {children ?? 'Your Support-tier account has read-only access to this area.'}
        </span>
      </div>
    </Card>
  );
