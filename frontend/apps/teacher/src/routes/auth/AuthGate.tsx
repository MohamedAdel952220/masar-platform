...
import { canOpenTeacherApp } from '../../lib/teacher';
import { UnauthenticatedFlow } from './UnauthenticatedFlow';

/**
 * Entry gate for the Teacher App.
 *
 *   1. session resolving   → spinner
 *   2. no session          → login
 *   3. role is not teacher → wrong-app notice
 *   4. otherwise           → the app
 *
...
export function AuthGate({ children }: { children: ReactNode }) {
  ...
  if (status !== 'authenticated') return <UnauthenticatedFlow appName="Masar for Teachers" />;

  if (!canOpenTeacherApp(claims)) {
    return (
      <CenteredLayout>
        <Card padding="lg" accent="var(--amber-500)">
          <div style={{ display: 'grid', gap: 'var(--space-3)' }}>
            ...
            <p style={{ margin: 0, fontSize: 'var(--text-sm)', color: 'var(--text-body)' }}>
              This app is for teachers. Your account signs in to a different Masar app.
            </p>
